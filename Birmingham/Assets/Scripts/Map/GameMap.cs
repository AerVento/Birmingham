...
namespace Game.Map
{
    ...
    public class GameMap
    {
        ...
        /// <returns></returns>
        public T GetCity<T>(CityName city) where T: ICity
        {
            return (T)_cities[city].City;
        }
        ...
        /// <returns>路信息。如果没有这条路，则返回null。</returns>
        public Road GetRoad(CityName cityA, CityName cityB)
        {
            if(_roads.TryGetValue((cityA, cityB), out var road))
            {
                return road;
            }
            return null;
        ...
        /// </summary>
        /// <returns></returns>
        public bool IsConnected(CityName a, CityName b)
        {
            if (a == b)
                return true;
            
            foreach(var neighbour in _cities[a].City.Neighbours)
            {
                if (_roads.ContainsKey((a, neighbour)) && IsConnected(neighbour, b)) // 递归的查找
                    return true;
            }
        ...
        public ProgramMessage CreateBuilding(PlayerEnum player, TechType type, CityName city, ushort index, IReadOnlyTechNode tech, bool checkIndustrial = false, bool isCheckOnly = false)
        {
            ...
            // 检查工业区情况
            if (checkIndustrial && _industrial[player].Count > 0 && !_industrial[player].Contains(city))
            {
